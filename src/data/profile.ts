...
export const experiences: Experience[] = [
  {
    ...
    location: 'Remote',
    highlights: [
      'Designing and delivering production-grade AI, ML, and GenAI solutions from architecture to deployment, including agentic AI systems, multi-agent workflows, and LLM applications powered by RAG, vector search, and cloud-native technologies',
      'Building end-to-end AI applications with a focus on scalability, observability, AgentOps, LLMOps, MLOps, evaluation, and production readiness',
      'Partnering with clients to translate business objectives into practical AI solutions, technical roadmaps, and implementation strategies',
      'Advising organizations on AI product strategy, feasibility assessment, roadmap prioritization, and responsible AI adoption',
      'Delivering corporate AI training programs, executive briefings, and hands-on technical workshops for engineering teams and business leaders',
...
export const talks: Talk[] = [
  ...
  },
  {
    ...
    description: 'A technical talk on the Gemini Managed Agents API: how a single client.interactions.create() call spins up a fully autonomous agent, model plus harness, inside a managed Linux sandbox. Covers the three surfaces it runs on (Google AI Studio, the Gemini API, and the Agent Platform), persisting reusable configuration with AGENTS.md, SKILL.md, and agents.create(), and multi-turn refinement with previous_interaction_id, illustrated with three live demos, one per surface: building an agent in Google AI Studio, the Daily Digest app on the Gemini API, and the Managed Issue Resolver on the Agent Platform.',
    highlights: [
      'The Foundation: an agent is a model plus a harness, sandbox, tools, instructions, and a loop',
      'One API Call: client.interactions.create() with streaming, inline environment.sources, and multi-turn refinement via previous_interaction_id',
      'Three Surfaces, three live demos: building an agent in Google AI Studio, the Daily Digest app on the Gemini API, and the Managed Issue Resolver on the Agent Platform',
      'Live demo: Daily Digest, a full-stack FastAPI + React app where one Gemini API call fetches tech news, writes editorial summaries, and exports a PDF',
      'Live demo: Managed Issue Resolver, a GitHub Actions pipeline where one agent fixes a labelled issue and opens a PR, and a second agent canary-deploys the merge to Cloud Run with automatic rollback',
      'Custom Agents: persisting reusable configuration with AGENTS.md, SKILL.md, and agents.create()',
      'Sandbox internals: Ubuntu, Python, Node, 4 CPU / 16 GB RAM, and a 15-minute idle-to-snapshot lifecycle',
      'Eight real-world use-case patterns, from research assistants to full-stack code generation',
      'Pricing, quotas, and limits to plan for when running agents at scale',
      'Live Q&A on building agents with the Managed Agents API',
    ],
    sessions: [
      {
