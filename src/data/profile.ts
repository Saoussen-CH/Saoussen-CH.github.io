...
export const experiences: Experience[] = [
  {
    ...
    startDate: '2023',
    endDate: 'Present',
    title: 'AI Solutions Engineer & Architect',
    company: 'Independent',
    location: 'Remote',
    highlights: [
      'Designing and delivering production-grade AI, ML, and GenAI solutions from architecture to deployment: agentic systems, multi-agent workflows, and LLM applications',
      'Building end-to-end AI applications focused on scalability, observability, AgentOps, LLMOps, MLOps, and production readiness',
      'Partnering with clients to translate business objectives into AI solutions, technical roadmaps, and implementation strategies',
      'Advising organizations on AI product strategy, feasibility assessment, and responsible AI adoption',
      'Delivering corporate AI training programs and hands-on technical workshops for engineering and business leaders',
      'Sharing knowledge through technical writing, open-source projects, conference talks, and community engagement',
    ],
  },
  {
    id: 'sofrecom',
...
export const talks: Talk[] = [
  ...
  {
    id: 'talk-managed-agents-api',
    emoji: '🎤',
    type: 'Talk',
    title: 'Managed Agents: Build Agents with One API Call, a Full Sandbox, and Three Surfaces',
    description: 'Before Managed Agents, building a production-grade AI agent meant weeks of infrastructure work: provisioning sandboxes, implementing reasoning loops, and managing state and multi-turn memory. This talk introduces the Gemini Managed Agents API and the shift it represents: a single API call provisions a fully autonomous agent powered by the Antigravity harness - the same harness behind Google\'s Deep Research - and Gemini 3.5 Flash inside a managed Linux sandbox with full filesystem access, code execution, and web browsing. The talk covers the model + harness foundation, customisation with AGENTS.md and SKILL.md, named agent persistence, and multi-turn refinement across all three surfaces (AI Studio, Gemini API, and Agent Platform), brought to life with three live demos: AI Studio templates, a full-stack Daily Digest app, and the Managed Issue Resolver where two agents autonomously fix a GitHub issue and canary-deploy the fix to Cloud Run with automatic rollback.',
    highlights: [
      'The Foundation: the Antigravity harness wraps Gemini 3.5 Flash with a ReAct loop, tool execution, and a managed Linux sandbox',
      'Before vs. After: from weeks of harness infrastructure to a single API call that provisions a full Ubuntu sandbox with 4 CPU / 16 GB RAM, Python, Node.js, filesystem, and web access',
      'Multi-turn refinement: resume both filesystem state and conversation history across API calls via environment ID and previous_interaction_id',
      'Three Surfaces: AI Studio for no-code prototyping, Gemini API with open network for developer integration, Agent Platform with IAM and network allowlists for enterprise production',
      'Custom Agents: AGENTS.md for system instructions, SKILL.md for reusable procedural knowledge',
      'agents.create() for persistent, named, team-shareable agent configurations',
      'Managed Agents vs ADK: when to use the fully managed harness to ship fast vs. when to own the harness with ADK for maximum control, on-prem deployment, and sub-second latency requirements',
      'Live demo: Daily Digest, a full-stack FastAPI + React app where one Gemini API call fetches tech news, writes editorial summaries, and exports a polished PDF streamed live to the browser',
      'Live demo: Managed Issue Resolver on Agent Platform, a two-agent GitHub Actions pipeline where one agent reads a labelled GitHub issue via GitHub MCP, clones the repo, fixes the bug, runs tests, and opens a PR; a second agent canary-deploys the merged fix to Cloud Run and monitors error rates via Cloud Monitoring MCP with automatic rollback',
    ],
    sessions: [
      {
      ...
      },
      {
        ...
        eventUrl: 'https://gdg.community.dev/events/details/google-gdg-damanhour-presents-io-extended-dive-into-the-future-of-googles-innovations-2026/',
      },
    ],
  },
  {
  ...
  {
    id: 'codelab-tech-digest-managed-agents',
    codelabUrl: 'https://codelabs.developers.google.com/tech-digest-managed-agent',
    emoji: '📰',
    type: 'Workshop',
    title: 'Build a Daily Tech Digest Agent with Managed Agents on the Gemini API',
    description: 'Managed agents on the Gemini API give you a configurable agent harness: a single API call provisions a Google-hosted Linux sandbox where the agent reasons, executes code, manages files, and browses the web autonomously. Unlike a standard LLM call that returns text, you get a persistent compute environment where the agent acts and produces file artifacts, with nothing to deploy or manage. This hands-on codelab, built in collaboration with Mete Atamel (Developer Advocate at Google) and published on Google Codelabs, is a practical introduction to these concepts. Using a tech digest as the running example, you go from a blank Python file to a working agent that fetches today\'s top stories, writes editorial summaries in a configured voice, generates a PDF, delivers it via Gmail, and runs on a daily schedule.',
    highlights: [
      'What is a managed agent: one API call provisions a Google-hosted Linux sandbox with code execution, web search, URL fetching, and filesystem tools, nothing to deploy or manage',
      'First agent call: client.interactions.create() with streaming to watch the agent reason and act in real time',
      'Customize with AGENTS.md: mount instructions and editorial voice directly into the sandbox filesystem at startup',
      'Skills: extend the agent with SKILL.md and a pre-built PDF renderer, letting the agent discover and run them autonomously',
      'Safety hooks: hooks.json + gate.py intercept tool calls before they execute; a deny response cancels the call and the agent self-corrects',
      'Environment persistence: the sandbox retains files and state for 7 days, your handle back to it is environment_id',
      'Multi-turn refinement: resume the same sandbox with environment_id and previous_interaction_id to refine the digest without re-fetching the web',
      'Save as a named agent: agents.create() bakes your configuration on Google\'s side so future calls need no re-upload',
      'Gmail delivery: OAuth 2.0 + the Gmail REST API to send the PDF digest directly to your inbox',
      'Daily scheduling: triggers.create() with a cron schedule so the agent runs and emails the digest automatically every day',
    ],
    sessions: [],
  },
  {
    id: 'workshop-managed-agents-issue-resolver',
    emoji: '🔧',
    type: 'Workshop',
    title: 'Autonomous GitHub Issue Resolution with the Google Managed Agents API',
    description: 'A hands-on, code-first codelab where you build a fully autonomous, two-agent CI/CD pipeline on the Agent Platform. Label a GitHub issue ai-resolve and a Resolver Agent reads it via GitHub MCP, clones the repo, fixes the bug, runs the tests, and opens a PR. When the PR merges, a CD Agent deploys the fix to Cloud Run with canary traffic splitting, polls Cloud Monitoring MCP for error rates, and promotes or rolls back automatically, using Cloud Logging MCP for context. No orchestration framework and nothing to deploy: just two AGENTS.md files, two SKILL.md files, and two Python scripts calling the Managed Agents API.',
    highlights: [
      'What is the Managed Agents API: one call provisions a full Linux sandbox (Bash, Python, git, web access), nothing to deploy or manage',
      'Write the Resolver Agent: reads a labelled GitHub issue via GitHub MCP, clones the repo, fixes the bug, runs the tests, and opens a PR',
      'Write the CD Agent: deploys via canary traffic splitting, polls Cloud Monitoring MCP, and promotes or rolls back automatically',
      'Create named agents on the Agent Platform with agents.create(), passing AGENTS.md as system_instruction and uploading SKILL.md to GCS as the agent home',
      'Hosted MCP servers for GitHub, Cloud Monitoring, and Cloud Logging, no MCP servers to deploy or manage',
      'Two GitHub Actions workflows: resolve.yml triggers on the ai-resolve label, deploy.yml triggers on merge to master',
      'Trigger end-to-end issue resolution and watch the agent fix four seeded bugs in a conference-session browser app',
      'Review the agent-authored PR, merge it, and watch the CD agent canary-deploy the fix and close the issue automatically',
    ],
    sessions: [],
  },
];
